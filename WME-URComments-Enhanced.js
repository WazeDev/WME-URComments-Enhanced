...
/* global unsafeWindow, $, GM_info, I18n, OpenLayers, W, WazeWrap */
...
(function () {
    ...
    const SCRIPT_NAME = `URC-E${((GM_info.script.name.search(/beta/) > -1) ? ' β' : (GM_info.script.name.search(/\(DEV\)/i) > -1) ? ' Ω' : '')}`,
        ...
        SCRIPT_VERSION_CHANGES = ['<b>CHANGE:</b> New linter options',
            '<b>CHANGE:</b> Restore tab clicking in latest WME.'
        ],
    ...
    function autoSwitchToUrceTab() {
        ...
        document.querySelector('a[href="#sidepanel-urc-e"]').dispatchEvent(new MouseEvent('click', {
            view: unsafeWindow, bubbles: true, cancelable: true, button: 0
        }));
