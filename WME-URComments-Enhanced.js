...
const SCRIPT_NAME = GM_info.script.name.replace('(beta)', 'β'),
      ...
      SCRIPT_VERSION_CHANGES = ['<b>BUGFIX:</b> Recenter on UR zoomed to 5 if you were zoomed to 0.' ],
...
let _settings = {},
    ...
    _restoreZoom, _$restoreTab, _restoreTabPosition, _wmeUserId, _initUrIdInUrlObserver, _lastUrOverflowCount, gapiAuth2, gapiUser;
...
function buildAlertBoxFromArray() {
    _alertBoxInUse = true;
    let currBoxObj = _alertBoxArray.shift();
...
function showAlertBanner(message, delay, mapBanner, panelBox, panelBoxTitle, panelBoxDismiss, index, phase) {
    ...
    if (panelBox) {
        ...
        $('#panel-urce-comments').prepend(
            $('<div>', {id:`urceAlertPanelBox-${index}`, class:'URCE-divWarningBox', title:`${(panelBoxTitle ? panelBoxTitle : '')}`})
        );
        if (panelBoxDismiss) {
    ...
    return new Promise((resolve) => {
        ...
        if (_restrictions[country]) {
            if (state && _restrictions[country][state]) {
                ...
                Object.keys(_restrictions[country][state]).map((restriction) => {
                    ...
                    restrictionsAlertBannerTitle += '\n' + I18n.t('urce.prefs.' + restriction.charAt(0).toUpperCase() + restriction.slice(1)) + ': ' +
                        ((_restrictionsEnforce[restriction] === true) ? I18n.t('urce.common.Enabled') : (_restrictionsEnforce[restriction] === false) ? I18n.t('urce.common.Disabled') : I18n.t('common.time.days', {days:_restrictionsEnforce[restriction]}));
                });
            }
                ...
                Object.keys(_restrictions[country].ALL).map((restriction) => {
...
async function handleUpdateRequestContainer(urId, caller) {
    ...
    if (_settings.autoSwitchCommentList) {
        if (_autoSwitch[W.model.countries.top.abbr] && ((_autoSwitch[W.model.countries.top.abbr].ALL > -1) || (_autoSwitch[W.model.countries.top.abbr][W.model.states.top.name] > -1))) {
            let commentList = (_autoSwitch[W.model.countries.top.abbr][W.model.states.top.name] > -1) ?
                _autoSwitch[W.model.countries.top.abbr][W.model.states.top.name] :
            (_autoSwitch[W.model.countries.top.abbr].ALL) ?
                _autoSwitch[W.model.countries.top.abbr].ALL : -1;
    ...
    $('#panel-container .mapUpdateRequest .top-section .header .title .focus').off('click', recenterOnUr).on('click', {urId:urId}, recenterOnUr);
    ...
    if (W.model.mapUpdateRequests.objects[urId].attributes.urceData.commentCount === 0) {
        ...
        if (_selUr.urOpen && commentNum) {
            if (((_settings.perCommentListSettings[_currentCommentList].autoSetNewUrComment || (_restrictionsEnforce.autoSetNewUrComment === true)) && (_restrictionsEnforce.autoSetNewUrComment !== false) && !W.model.mapUpdateRequests.objects[urId].attributes.description) ||
                ((_settings.perCommentListSettings[_currentCommentList].autoSetNewUrCommentWithDescription || (_restrictionsEnforce.autoSetNewUrCommentWithDescription === true)) && (_restrictionsEnforce.autoSetNewUrCommentWithDescription !== false) && W.model.mapUpdateRequests.objects[urId].attributes.description && (W.model.mapUpdateRequests.objects[urId].attributes.type !== 23)) ||
                ((_settings.perCommentListSettings[_currentCommentList].autoSetNewUrCommentSlur || (_restrictionsEnforce.autoSetNewUrCommentSlur === true)) && (_restrictionsEnforce.autoSetNewUrCommentSlur !== false) && W.model.mapUpdateRequests.objects[urId].attributes.type === 23)) {
        ...
        if (_selUr.urOpen && (_settings.perCommentListSettings[_currentCommentList].autoSetReminderUrComment || (_restrictionsEnforce.autoSetReminderUrComment === true)) && (_restrictionsEnforce.autoSetReminderUrComment !== false) && _defaultComments.dr.commentNum &&
            (W.model.mapUpdateRequests.objects[urId].attributes.urceData.commentCount > 0) && (_settings.perCommentListSettings[_currentCommentList].reminderDays !== 0) && (_restrictionsEnforce.reminderDays !== 0) &&
            ((W.model.mapUpdateRequests.objects[urId].attributes.urceData.lastCommentDaysOld > (_settings.perCommentListSettings[_currentCommentList].reminderDays - 1)) || W.model.mapUpdateRequests.objects[urId].attributes.urceData.lastCommentDaysOld > (_restrictionsEnforce.reminderDays - 1)) &&
    ...
    if (_settings.autoCenterOnUr)
        recenterOnUr({data:{urId:urId}}, W.map.getZoom());
    if ($('#urceShortcuts').length === 0) {
        $('#panel-container .mapUpdateRequest .top-section .body .conversation .new-comment-form').prepend(
            $('<div>', {id:'urceShortcuts', style:'text-align:center; padding-bottom:8px;'}).append(
                $('<div>').append(
                    $('<i>', {class:'fa fa-road', 'aria-hidden':'true', style:'cursor:pointer; padding-left:4px;', title:I18n.t('urce.urPanel.InsertSelSegsTitle')}).off().on('click', {shortcut:'selSegs'}, handleClickedShortcut),
                    $('<i>', {class:'fa fa-clock-o', 'aria-hidden':'true', style:'cursor:pointer; padding-left:4px;', title:I18n.t('urce.urPanel.InsertTimeTitle')}).off().on('click', {shortcut:'time'}, handleClickedShortcut),
                    $('<i>', {class:'fa fa-sun-o', 'aria-hidden':'true', style:'cursor:pointer; padding-left:4px;', title:I18n.t('urce.urPanel.InsertDayOfWeekTitle')}).off().on('click', {shortcut:'dayOfWeek'}, handleClickedShortcut),
                    $('<i>', {class:'fa fa-calendar-o', 'aria-hidden':'true', style:'cursor:pointer; padding-left:4px;', title:I18n.t('urce.urPanel.InsertDateTitle')}).off().on('click', {shortcut:'date'}, handleClickedShortcut),
                    $('<i>', {class:'fa fa-paragraph', 'aria-hidden':'true', style:'cursor:pointer; padding-left:4px;', title:I18n.t('urce.urPanel.InsertDescriptionTitle')}).off().on('click', {shortcut:'description'}, handleClickedShortcut),
                    $('<i>', {class:'fa fa-user-o', 'aria-hidden':'true', style:'cursor:pointer; padding-left:4px;', title:I18n.t('urce.urPanel.InsertWazeUsernameTitle')}).off().on('click', {shortcut:'wazeUsername'}, handleClickedShortcut)
                ).append(
                    $('<div>', {style:'padding-left:20px; display:inline-block;'}).append(
                        $('<i>', {class:'fa fa-clock-o', 'aria-hidden':'true', style:'cursor:pointer; padding-left:4px;', title:I18n.t('urce.urPanel.InsertTimeCasualTitle')}).off().on('click', {shortcut:'timeCasual'}, handleClickedShortcut),
                        $('<i>', {class:'fa fa-calendar', 'aria-hidden':'true', style:'cursor:pointer; padding-left:4px;', title:I18n.t('urce.urPanel.InsertDateCasualTitle')}).off().on('click', {shortcut:'dateCasual'}, handleClickedShortcut)
                    )
                )
            )
        );
        ...
        if (text.search(/(\$DRIVEDATE_DAY_OF_WEEK\$|\$DRIVEDATE_DATE\$|\$DRIVEDATE_DATE_CASUAL\$|\$DRIVEDATE_DAYS_AGO\$|\$DRIVEDATE_TIME\$|\$DRIVEDATE_TIME_CASUAL\$)/gm) > -1) {
            if (W.model.mapUpdateRequests.objects[_selUr.urId]) {
                ...
                if (text.indexOf('$DRIVEDATE_DATE_CASUAL$') > -1) {
                    if (W.model.mapUpdateRequests.objects[_selUr.urId].attributes && (W.model.mapUpdateRequests.objects[_selUr.urId].attributes.driveDate > -1))
                        text = text.replace('$DRIVEDATE_DATE_CASUAL$', new Date(W.model.mapUpdateRequests.objects[_selUr.urId].attributes.driveDate).toLocaleDateString(I18n.currentLocale(), {month:'long', day:'2-digit'}));
                    else
                ...
                if (text.indexOf('$DRIVEDATE_TIME$') > -1) {
                    if (W.model.mapUpdateRequests.objects[_selUr.urId].attributes && (W.model.mapUpdateRequests.objects[_selUr.urId].attributes.driveDate > -1))
                        text = text.replace('$DRIVEDATE_TIME$', new Date(W.model.mapUpdateRequests.objects[_selUr.urId].attributes.driveDate).toLocaleTimeString(I18n.currentLocale(), {hour:'2-digit', minute:'2-digit', timeZoneName:'short'}));
                    else
...
function recenterOnUr(event, zoom) {
    let urId = (event.data && event.data.urId) ? event.data.urId : -1;
    zoom = (zoom === 0) ? 0 : (zoom) ? zoom : 5;
    if (urId < 0)
        return;
...
function updateUrMapMarkers(urIds, filter) {
    let closeDays = _restrictionsEnforce.closeDays || _settings.perCommentListSettings[_currentCommentList].closeDays || 7;
    let markerChanges = {
        markers: {
            hidden: [],
            unhidden: [],
...
function updateUrceData(urIds) {
    ...
    let reminderDays = _restrictionsEnforce.reminderDays || _settings.perCommentListSettings[_currentCommentList].reminderDays || 0;
    let closeDays = _restrictionsEnforce.closeDays || _settings.perCommentListSettings[_currentCommentList].closeDays || 7;
                    ...
                    if (mapUrsObj[idx].attributes.open && urceData.commentCount === 1) {
                        if ((reminderDays !== 0) && (urceData.lastCommentDaysOld > (reminderDays - 1))) {
                            if ((_settings.perCommentListSettings[_currentCommentList].autoSendReminders || _restrictionsEnforce.autoSendReminders === true) &&
                                (_restrictionsEnforce.autoSendReminders !== false) && (urceData.lastCommentBy > 1) && (_wmeUserId === urceData.lastCommentBy) && !mapUrsObj[idx].attributes.reminderSent) {
                                try {
...
async function handleUrOverflow() {
    ...
    let vpBounds = W.map.getExtent().transform(W.map.projection, W.map.displayProjection);
    let vpBoundsFrom =  { lon:vpBounds.left, lat:vpBounds.bottom };
    let vpBoundsTo = { lon:vpBounds.right, lat:vpBounds.top };
    let vpCenter = W.map.getCenter().transform(W.map.projection, W.map.displayProjection);
    ...
    while (overflowUrlsToCheck.length > 0) {
        ...
        respUrObjs.forEach((respUrObj) => {
            if (W.model.mapUpdateRequests.objects[respUrObj.id] === undefined) {
                ...
                toPutPoint.transform(W.map.projection, W.map.displayProjection);
...
function invokeMoveEnd() {
    if (_settings.enableAutoRefresh && (W.map.getZoom() > 2) && (W.model.mapUpdateRequests.getObjectArray().length > 499) && (W.saveController.options.actionManager.events.object._undoStack.length === 0))
...
async function changeCommentList(commentListIdx, autoSwitch, refresh) {
    ...
    if (refresh || (!autoSwitch && ((commentListIdx !== _settings.commentList) || (commentListIdx !== _currentCommentList))) || (autoSwitch && (commentListIdx !== _currentCommentList))) {
        ...
        if (buildCommentListResult.error)
            handleError(buildCommentListResult.error, (getCommentListInfo(commentListIdx).type === 'static'), 'changeList', (_selUr.urId > 0), buildCommentListResult.commentList);
        if (!autoSwitch && !refresh)
            saveSettingsToStorage();
    }
...
function setGapiSigninStatus() {
    gapiUser = gapiAuth2.currentUser.get();
    if (gapiUser.hasGrantedScopes('https://www.googleapis.com/auth/drive.file https://www.googleapis.com/auth/drive.readonly')) {
...
function makeApiRestCall(obj) {
    return new Promise(async (resolve) => {
        let data, token = gapiUser.getAuthResponse().access_token;
        try {
            let options = {...obj};
...
async function createStaticToGoogleSheet(convert) {
    ...
    if (_alertBoxArray.length > 0) {
        for(let idx = 0; idx < _alertBoxArray.length; idx++) {
            if ((_alertBoxArray[idx].tickAction === 'disabled') && ((_alertBoxArray[idx].title === I18n.t('urce.prompts.ConvertToGoogleSheet')) || (_alertBoxArray[idx].title === I18n.t('urce.prompts.CreateGoogleSheet'))))
    ...
    let content = '<div id="urceCreateConvertResults">' +
        ...
        '<div><b>' + I18n.t('urce.prompts.ConvertCreateGoogleAccount') + ':</b> <i>' + gapiUser.getBasicProfile().getEmail() + '</i></div>' +
...
function commentListAsync(commentListIdx) {
    ...
    return new Promise(async (resolve, reject) => {
        let data = await $.getJSON(`https://sheets.googleapis.com/v4/spreadsheets/${((commentListIdx === 1001) ? _settings.customSsId : URCE_SPREADSHEET_ID)}/values/${commentListInfo.gSheetRange}?key=${URCE_API_KEY}`).fail((response) => {
...
async function buildCommentList(commentListIdx, phase, autoSwitch) {
    ...
    $('#_commentList').append(
        ...
        $('<div>', {id:'expandCollapseAll', class:'URCE-expandCollapseAll'}).append(
            $('<div>', {class:'URCE-expandCollapseAllItem'}).text(I18n.t('urce.common.ExpandAll')).off().on('click', () => {
                let $legends = $('legend[id^="urceComments-for"');
    ...
    try {
        ...
        return {error:error, staticList:(commentListInfo.type === 'static'), phase:phase, maskUrPanel:(_selUr.urId > 0), commentList:commentListIdx};
    }
    try {
    ...
    return {error:undefined, staticList:(commentListInfo.type === 'static'), phase:phase, maskUrPanel:(_selUr.urId > 0), commentList:commentListIdx};
}
...
function processPerCommentListSettings(commentListIdx) {
    ...
    if (_settings.perCommentListSettings[commentListIdx] === undefined) {
        ...
        for (let prop in defaultPerCommentListSettings) {
    ...
    if (perCommentListSettingsChanged)
        saveSettingsToStorage();
    let perCListSettings = _settings.perCommentListSettings[commentListIdx];
    let htmlOut = '' +
        ...
        '      <input type="checkbox" style="margin-left:8px;" id="_cbperCommentList_autoSendReminders_useDefault" urceprefs="perCommentList" class="urceSettingsCheckbox" title="' + I18n.t('urce.prefs.UseDefault') + '" ' + ((perCListSettings.autoSendReminders_useDefault) ? 'checked="true"' : '') + '>' +
        '   </div>' +
        '   <div>' +
        ...
        '      <input type="checkbox" style="margin-left:8px;" id="_cbperCommentList_autoSetNewUrCommentWithDescription_useDefault" urceprefs="perCommentList" class="urceSettingsCheckbox" title="' + I18n.t('urce.prefs.UseDefault') + '" ' + ((perCListSettings.autoSetNewUrCommentWithDescription_useDefault) ? 'checked="true"' : '') + '>' +
        '   </div>' +
        '   <div>' +
        ...
        '      <input type="checkbox" style="margin-left:8px;" id="_cbperCommentList_autoSetReminderUrComment_useDefault" urceprefs="perCommentList" class="urceSettingsCheckbox" title="' + I18n.t('urce.prefs.UseDefault') + '" ' + ((perCListSettings.autoSetReminderUrComment_useDefault) ? 'checked="true"' : '') + '>' +
        '   </div>' +
        '</div>' +
        '<div class="URCE-controls URCE-textFirst">' +
        '   <div>' +
        '      <div style="display:inline;" title="' + I18n.t('urce.prefs.TagEmailTitle') + '" class="URCE-label' + ((perCListSettings.tagEmail_useDefault) ? ' urceDisabled' : '') + '">' +
        I18n.t('urce.prefs.TagEmail') +
        '      </div>' +
        ...
        '      <input type="checkbox" style="margin-left:8px;" id="_cbperCommentList_tagEmail_useDefault" urceprefs="perCommentList" class="urceSettingsCheckbox" title="' + I18n.t('urce.prefs.UseDefault') + '" ' + ((perCListSettings.tagEmail_useDefault) ? 'checked="true"' : '') + '>' +
        '   </div>' +
        '   <div>' +
        '      <div style="display:inline;" title="' + I18n.t('urce.prefs.ReminderDaysTitle') + '" class="URCE-label' + ((perCListSettings.reminderDays_useDefault) ? ' urceDisabled' : '') + '" urceprefs="perCommentList">' +
        I18n.t('urce.prefs.ReminderDays') +
        '      </div>' +
        ...
        '      <input type="checkbox" style="margin-left:8px;" id="_cbperCommentList_reminderDays_useDefault" urceprefs="perCommentList" class="urceSettingsCheckbox" title="' + I18n.t('urce.prefs.UseDefault') + '" ' + ((perCListSettings.reminderDays_useDefault) ? 'checked="true"' : '') + '>' +
        '   </div>' +
        '   <div>' +
        '      <div style="display:inline;" title="' + I18n.t('urce.prefs.CloseDaysTitle') + '" class="URCE-label' + ((perCListSettings.closeDays_useDefault) ? ' urceDisabled' : '') + '" urceprefs="perCommentList">' +
        I18n.t('urce.prefs.CloseDays') +
        '      </div>' +
        ...
        '      <input type="checkbox" style="margin-left:8px;" id="_cbperCommentList_closeDays_useDefault" urceprefs="perCommentList" class="urceSettingsCheckbox" title="' + I18n.t('urce.prefs.UseDefault') + '" ' + ((perCListSettings.closeDays_useDefault) ? 'checked="true"' : '') + '>' +
        '   </div>' +
        '</div>';
    $('#URCE-divPerCommentListSettings').html(htmlOut);
    $('input[urceprefs="perCommentList"]').off().on('change', function() {
        let settingName = $(this)[0].id.replace(/(_.+perCommentList_)/gmi, '');
        if (this.type === 'checkbox') {
            _settings.perCommentListSettings[_currentCommentList][settingName] = isChecked(this);
            saveSettingsToStorage();
        }
        if (this.type === 'number') {
            let val = Math.min(9999, Math.max(0, Math.abs((parseInt(this.value) || 0))));
        ...
        if (settingName.indexOf('_useDefault') > -1) {
            ...
            let parentSettingName = settingName.replace('_useDefault', '');
...
function handleError(error, staticList, phase, maskUrPanel, commentList) {
    ...
    if (error.indexOf('|') > -1) {
        ...
        errorText = error;
        outputText = (commentList === 1001) ? I18n.t('urce.prompts.CustomGSheetLoadError') : I18n.t('urce.common.ErrorGeneric');
    }
    logError(errorText);
    _currentCommentList = null;
    if (phase === 'changeList') {
        if (staticList)
            ...
            if (commentList) {
                let commentListInfo = getCommentListInfo(commentList);
    ...
    if (status === 'enable') {
        ...
        gapi.load('client:auth2', function() {
            gapi.client.init({
                ...
                gapiAuth2.isSignedIn.listen(updateGapiSigninStatus);
                gapiUser = gapiAuth2.currentUser.get();
                setGapiSigninStatus();
        ...
        $('<fieldset>', {id:'urce-prefs-fieldset-commentList', class:`URCE-field${urStyle}`}).append(
            ...
            $('<div>', {class:'URCE-controls URCE-divCC'}).append(
                ...
                $('<div>').text(I18n.t('urce.prefs.CustomSsId') + ':').append(
                    $('<input>', {type:'text', id:'_textcustomSsId', class:'URCE-textInput urceSettingsTextBox', urceprefs:'commentList', value:_settings.customSsId, title:I18n.t('urce.prefs.CustomSsIdTitle'), style:'width:100px; margin-left:5px;'})
                ),
                // Comment list style
    ...
    $('.urceSettingsTextBox').off().on('change', function() {
        ...
        if ((val !== this.value) || (_settings[settingName] !== val)) {
            ...
            if ((settingName !== 'tagEmail') && (settingName !== 'customSsId'))
                ...
                if ((settingName === 'tagEmail') || ((settingName === 'customSsId') && (_currentCommentList === 1001)))
    ...
    return new Promise(async (resolve) => {
        ...
        if (data && data.values.length > 0) {
            for (let entryIdx = 0; entryIdx < data.values.length; entryIdx++) {
                ...
                if (data.values[entryIdx].length < 2)
                    continue;
                let restriction = data.values[entryIdx][1];
...
async function init() {
    ...
    let initRestrictionsError = await initRestrictions();
    await initGui();
    maskBoxes(I18n.t('urce.prompts.WaitingOnInit') + '.<br>' + I18n.t('urce.common.PleaseWait') + '.', false, 'init', (urIdInUrl > 0));
    let errorText = loadTranslationsError.error || initCommentListsError.error || initAutoSwitchArraysError.error || initRestrictionsError.error || undefined;
    if (!errorText) {
        await checkRestrictions();
        let buildCommentListResult = await buildCommentList(undefined, 'init', false);
        if (buildCommentListResult.error)
            handleError(buildCommentListResult.error, buildCommentListResult.static, buildCommentListResult.phase, (urIdInUrl > 0), buildCommentListResult.commentList);
        else if (!urIdInUrl)
            await initBackgroundTasks('enable');
        ...
        handleError(errorText, false, 'init', (urIdInUrl > 0), null)
}
            ...
            translations = {
                "en": {
                    ...
                    "prefs": {
                        ...
                        "DefaultListTitle": "Select the custom list you would like to use. CommentTeam is the default. If you would like your comment list built into this script or have suggestions on the CommentTeam list, please contact a WazeDev team member.",
                    ...
                    "urPanel": {
                        ...
                        "InsertDateTitle": "Shortcut - Drive date: Click this icon to insert the drive date into the new comment box at the cursor position (2-digit month, 2-digit day, 4-digit year in locale format).",
